...
## Estimating process variances

DFA models generally constrain process errors to be normally distributed with unit variance, e.g. 

$$delta_{i,t=1:T} \sim Normal( 0, q)$$
While we generally recommend following this convention, there are some combinations of models and parameters where $q$ is identifiable. For these models, we can estimate the process error variance as

```{r echo=TRUE, eval=FALSE}
fit <- fit_dfa(..., estimate_process_sigma = TRUE)
```

This form of the model allows separate process variances to be estimated on each trend, e.g. $q_{1}, q_{2}, q_{3}, ...$
