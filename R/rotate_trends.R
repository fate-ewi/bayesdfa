...
rotate_trends <- function(fitted_model, conf_level = 0.95, invert = FALSE) {
    ...
    list(Z_rot = flip * mcmc_Z_rot, trends = flip * mcmc_trends_rot, Z_rot_mean = apply(flip * mcmc_Z_rot,
        c(2, 3), mean), Z_rot_median = apply(flip * mcmc_Z_rot, c(2, 3), median), trends_mean = apply(flip *
        mcmc_trends_rot, c(2, 3), mean), trends_median = apply(flip * mcmc_trends_rot, c(2, 3), median),
        trends_lower = apply(flip * mcmc_trends_rot, c(2, 3), quantile, (1 - conf_level)/2), trends_upper = apply(flip *
            mcmc_trends_rot, c(2, 3), quantile, 1 - (1 - conf_level)/2))
}
