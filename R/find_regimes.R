...
find_regimes <- function(y, sds = NULL, min_regimes = 1, max_regimes = 3, iter = 2000, thin = 1, chains = 1, 
    ...) {
    df <- data.frame(regimes = seq(min_regimes, max_regimes), looic = NA)
    best_loo <- 1e+10
    best_model <- NA
    for (regime in seq(min_regimes, max_regimes)) {
        fit <- fit_regimes(y = y, sds = sds, n_regimes = regime, iter = iter, thin = thin, chains = chains, 
            ...)
    ...
    list(table = df, best_model = best_model, n_loo_bad = n_loo_bad, n_loo_very_bad = n_loo_very_bad)
}
