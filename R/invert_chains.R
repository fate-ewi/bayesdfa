...
find_inverted_chains <- function(model, trend = 1, plot = FALSE) {
    ...
    ## vv is dimensioned nchains * nyears (x[1:nyears,trend=i]) zz is dimensioned n_time series (Z[1:time
    ## series,trend=i])
    
    if (plot) {
        p <- ggplot(vv, aes_string("as.numeric(parameters)", "estimate", color = "chains")) + geom_line()
        print(p)
    }
    ...
    if (nchains > 1) {
        for (i in seq(2, nchains)) {
            ...
            if ((sum((-1 * pred1_loadings - pred0_loadings)^2) + sum((-1 * pred1_trend - pred0_trend)^2)) < 
                ...
                # flip this chain -- seems to be something not right with commented out line flipped_chains <-
                # ifelse(flipped_chains == 0, i, c(flipped_chains, i))
                if (flipped_chains == 0) {
                  flipped_chains <- i
                } else {
