#' Simulate from a DFA
#'
#' @param num_trends The number of trends, defaults to 2.
#' @param num_years The number of years, defaults to 20.
#' @param num_ts The number of timeseries, defaults to 4.
...
#'   \code{Z} in the returned list is to see the manipulated loadings matrix. If not
#'   specified, a random matrix ~ N(0, 1) is used.
#' @param sigma A vector of standard deviations on the observation error. Should
#'   be of the same length as the number of trends. If not specified, random numbers
#'   are used \code{rlnorm(1, meanlog = log(0.2), 0.1)}
#' @param varIndx Indices of unique observation variances. Defaults to c(1, 1, 1, 1)
#' where m is the number of time series. Unique observation error variances would be specified
#' as c(1, 2, 3, 4).
...
#' for all processes, and defaults to round(n_t/2) where n_t is the time series
#' length
#' @param nu_fixed Nu is the df parameter for the t-distribution, defaults to 100
#' @export
