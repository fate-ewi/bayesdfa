...
#' @export
#'
is_converged <- function(fitted_model, threshold = 1.05, parameters = c("sigma", "x", "Z")) {
    Rhats <- fitted_model$monitor[which(grepl(paste(parameters, collapse = "|"), rownames(fitted_model$monitor)) == 
        TRUE), "Rhat"]
