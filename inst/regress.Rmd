...
The following function fits Bayesian linear regressions predicting a DFA trend based on a climate predictor It cycles through 50 (or some other number) of samples from the DFA posterior.
...
trend_lm <- function(rotated_modelfit,
  ...
  ITER = 1000, CHAINS = 1, n_eff = 50, 
  rhat = 1.1, show_plot = TRUE) {
...
Here's an example of writing a function to include several covariates, for example if we wanted to test whether an coefficient changed slope over time.
...
For this example, we'll use the simple example in `rotate_trends`, 
...
Using this simple example, we can fit a model with a single covariate,

```{r}
cli_dat = data.frame("Year" = 1981:2000, "PDO" = rnorm(20))

mod1 = trend_lm(rotated_modelfit = r, all_years = 1981:2000,
  selected_years = 1981:2000, climate_dat=cli_dat)
summary(mod1)
```

Now we can expand and fit the model with three, including the interaction

```{r}
cli_dat = data.frame("Year" = 1981:2000, "PDO" = rnorm(20))
cli_dat$era = ifelse(cli_dat$Year < 1991,0,1)
cli_dat$interact = cli_dat$era*cli_dat$PDO

mod2 = trend_lm(rotated_modelfit = r, all_years = 1981:2000,
  selected_years = 1981:2000, climate_dat=cli_dat)
summary(mod2)
```

Last we can include 3 regimes, with only an interaction in the slope (constant intercept)
```{r}
cli_dat = data.frame("Year" = 1981:2000, "PDO" = rnorm(20))
cli_dat$regime = as.factor(sample(c(0,1,2), size=20, replace=T))
Y = rnorm(length(regime3))
cli_dat = as.data.frame(model.matrix(Y ~ Year + PDO:regime3, data=cli_dat))
...
boxplot(c(mod2[,c("PDOregime30","PDOregime31","PDOregime32")]) , sort(rep(1:3,nrow(mod2))), outline=FALSE)
```
